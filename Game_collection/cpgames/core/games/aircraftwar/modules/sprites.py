...
class Asteroid(pygame.sprite.Sprite):
    def __init__(self, cfg, resource_loader):
        pygame.sprite.Sprite.__init__(self)
        self.image = resource_loader.images['asteroid'].convert_alpha()
        # 位置
        self.rect = self.image.get_rect()
...
class Ship(pygame.sprite.Sprite):
    def __init__(self, idx, cfg, resource_loader):
        ...
        self.explode_image = resource_loader.images['ship_exploded'].convert_alpha()
        # 位置
    ...
    '''射击'''
    def shot(self):
        return Bullet(self.player_idx, (self.rect.center[0] - 5, self.position['y'] - 5), self.cfg, self.resource_loader)